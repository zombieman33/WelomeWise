...
public class WelcomeWiseCommand implements CommandExecutor, TabCompleter {
    ...
    public WelcomeWiseCommand(WelcomeWise plugin) {
        this.plugin = plugin;
    }
        ...
        if (args.length >= 1) {
            if (args[0].equalsIgnoreCase("help")) {
                ...
                if (player.hasPermission("welcomewise.command.message")) {
                    ...
                    plugin.saveConfig();
                    player.sendMessage("You set the new join message to " + ColorUtil.color(message));
                } else {
                    player.sendMessage(ChatColor.RED + "You don't have permission to run this command.");
                }
                ...
                if (player.hasPermission("welcomewise.command.broadcastmessage")) {
                    ...
                    plugin.saveConfig();
                    player.sendMessage("You set the new join broadcast message to " + ColorUtil.color(broadcastMessage));
                } else {
                    player.sendMessage(ChatColor.RED + "You don't have permission to run this command.");
                }
                ...
                if (player.hasPermission("welcomewise.command.firstmessage")) {
                    ...
                    plugin.saveConfig();
                    player.sendMessage("You set the new join first message to " + ColorUtil.color(message));
                } else {
                    player.sendMessage(ChatColor.RED + "You don't have permission to run this command.");
                }
                ...
                player.sendMessage(ChatColor.YELLOW + "/ws help");
            }
        }
        return true;
    }
